...
import com.shadowmask.core.BuildConfig.APP_PACKAGE_NAME
import com.shadowmask.core.Const
import com.shadowmask.core.download.DownloadNotifier
import com.shadowmask.core.download.DownloadProcessor
...
class Environment : BaseTest {
    ...
    private fun checkModuleZip(file: File) {
        ...
        ZipFile.Builder().setFile(file).get().use { zip ->
            ...
            assertArrayEquals(MODULE_ERROR, "#SHADOWMASK\n".toByteArray(), script)
        }
    }
    ...
    private fun setupSepolicyRuleModule(root: ExtendedFile) {
        ...
        PrintStream(path.getChildFile("sepolicy.rule").newOutputStream()).use {
            it.println("type shadowmask_test domain")
        }
    ...
    fun setupEnvironment() {
        runBlocking {
            assertTrue(
                "ShadowMask setup failed",
                ShadowMaskInstaller.Emulator(TimberLog, TimberLog).exec()
            )
        }
