package com.shadowmask.core.tasks
...
import com.shadowmask.core.AppApkPath
...
import com.shadowmask.core.ktx.copyAll
import com.shadowmask.core.ktx.writeTo
import com.shadowmask.core.utils.DataSourceChannel
import com.shadowmask.core.utils.DummyList
import com.shadowmask.core.utils.MediaStoreUtils
import com.shadowmask.core.utils.MediaStoreUtils.inputStream
import com.shadowmask.core.utils.MediaStoreUtils.openFd
...
abstract class ShadowMaskInstallImpl protected constructor(
    ...
    private val destName: String by lazy {
        if (Config.randName) {
            ...
            StringBuilder("shadowmask_patched-${BuildConfig.APP_VERSION_CODE}_").run {
                for (i in 1..5) {
            ...
            if (isRunningAsStub) {
                ZipFile.builder().setFile(StubApk.current(context)).get().use { zf ->
                    ...
                    if (Process.is64Bit() && abi32 != null) {
                        val entry = zf.getEntry("lib/$abi32/libshadowmask.so")
                        if (entry != null) {
                            val shadowmask32 = File(installDir, "shadowmask32")
                            zf.getInputStream(entry).writeTo(shadowmask32)
                        }
                    }
                }
            } else {
                ...
                // Also extract shadowmask32 on 64-bit devices that supports 32-bit
                val abi32 = Const.CPU_ABI_32
                if (Process.is64Bit() && abi32 != null) {
                    val name = "lib/$abi32/libshadowmask.so"
                    val entry = javaClass.classLoader!!.getResourceAsStream(name)
                    if (entry != null) {
                        val shadowmask32 = File(installDir, "shadowmask32")
                        entry.writeTo(shadowmask32)
                    }
                }
            }
            ...
            recovery != null -> {
                if (boot != null) {
                    ...
                    arrayOf(
                        "cd $installDir",
                        "chmod -R 755 .",
                        "./shadowmaskboot unpack boot.img",
                        "./shadowmaskboot repack boot.img",
    ...
    private fun patchBoot(): Boolean {
        ...
        shell.newJob().add("./shadowmaskboot cleanup", "cd /").exec()
...
) : ShadowMaskInstallImpl(console, logs) {
...
abstract class CallBackInstaller : ShadowMaskInstallImpl(DummyList, DummyList) {
