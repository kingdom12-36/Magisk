...
void show_print_help(void) {
	...
	log("      |--> Show various SUSFS kernel information\n");
	log("      |--> version: show SUSFS kernel version string\n");
	log("      |--> enabled_features: show all enabled SUSFS features in kernel\n");
	log("      |--> variant: show SUSFS variant (GKI or NON-GKI)\n");
	log("\n");
}
...
int show(int argc, char *argv[]) {
	...
	if (!strcmp(argv[2], "version")) {
		...
		struct st_susfs_enabled_features *info =
			malloc(sizeof(struct st_susfs_enabled_features));
		int err = 0;
