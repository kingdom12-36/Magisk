...
void enable_avc_log_spoofing_print_help(void) {
	...
	log("      |--> 0: disable spoofing the sus tcontext 'su' in avc log\n");
	log("      |--> 1: enable spoofing the sus tcontext 'su' with 'u:r:priv_app:s0:c512,c768'\n");
	log("      * Important Notes *\n");
...
int enable_avc_log_spoofing(int argc, char *argv[]) {
	...
	syscall(SYS_reboot, KSU_INSTALL_MAGIC1, SUSFS_MAGIC,
		CMD_SUSFS_ENABLE_AVC_LOG_SPOOFING, &info);
