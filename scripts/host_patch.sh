...
# full integration test of shadowmaskinit under several circumstances.
...
if $IS_RAMDISK; then
  ./shadowmaskboot decompress "$TARGET_FILE" ramdisk.cpio
else
  ./shadowmaskboot unpack "$TARGET_FILE"
fi
...
./shadowmaskboot compress=xz shadowmask shadowmask.xz
./shadowmaskboot compress=xz stub.apk stub.xz
./shadowmaskboot compress=xz init-ld init-ld.xz

./shadowmaskboot cpio ramdisk.cpio \
...
if $IS_RAMDISK; then
  ./shadowmaskboot compress=gzip ramdisk.cpio "$OUTPUT_FILE"
else
  ./shadowmaskboot repack "$TARGET_FILE" "$OUTPUT_FILE"
